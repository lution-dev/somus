...
| RN10 | **Conciliação mensal via extrato (pré-Open Finance).** Qualquer banco (99Pay, Inter, Nubank, Itaú, Santander ou genérico). Formatos **PDF, OFX ou CSV**. Crédito/débito pelo sinal; entrada pergunta renda vs divisão; saída pergunta divisão. Já lançado = só badge. Lançamento diário permanece. Sem fatura de cartão no v1. Ver `harness/plans/extrato-bancario.md`. |
