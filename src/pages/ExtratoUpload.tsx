...
export default function ExtratoUpload() {
  ...
  async function handleFile(file: File) {
    ...
    try {
      ...
      if (!format) {
        setError('Esse arquivo ainda não encaixa. Use OFX, OFC ou CSV.')
        return
      }
    ...
    <div style={{ paddingBottom: 40 }}>
      ...
      <div style={{ padding: '8px 16px 24px' }}>
        ...
        <p style={{
          ...
          Arquivo da conta corrente de {mesNome} em OFX ou CSV. Sem fatura de cartão.
        </p>
        <p style={{
          fontSize: 12, color: 'var(--color-text-tertiary)', margin: '0 0 24px',
        }}>
          Aceitos: OFX, OFC ou CSV.
        </p>

        <input
          ref={inputRef}
          type="file"
          accept=".ofx,.ofc,.csv,.txt,text/csv,application/x-ofx,application/vnd.intu.qfx"
          style={{ display: 'none' }}
          onChange={(e) => {
